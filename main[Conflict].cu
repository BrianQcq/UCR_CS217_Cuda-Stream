...
int main (int argc, char *argv[])
{
    ...
    */
    ...
    startTime(&timer);
    for(int i=0; i < VecSize; i += SegSize * StreamNum)
    {
        ...
        cudaMemcpyAsync(d_A1, h_A + i + 2 * SegSize, SegSize * sizeof(float), cudaMemcpyHostToDevice, stream2);
        cudaMemcpyAsync(d_B1, h_B + i + 2 * SegSize, SegSize * sizeof(float), cudaMemcpyHostToDevice, stream2);
